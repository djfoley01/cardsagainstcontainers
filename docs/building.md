...
```sh
podman tag cards-against-containers:0.1.0   docker.io/YOURUSER/cards-against-containers:0.1.0
```
...
```sh
podman tag cards-against-containers:0.1.0   docker.io/YOURUSER/cards-against-containers:latest
```

### 4. Push

```sh
podman push docker.io/YOURUSER/cards-against-containers:0.1.0
...
```sh
podman rmi docker.io/YOURUSER/cards-against-containers:0.1.0
podman pull docker.io/YOURUSER/cards-against-containers:0.1.0
```
...
```sh
helm install cac deploy/helm/cards-against-containers -n happyhour   --set image.repository=docker.io/YOURUSER/cards-against-containers   --set image.tag=0.1.0
```
...
helm upgrade cac deploy/helm/cards-against-containers -n happyhour   --set image.repository=docker.io/YOURUSER/cards-against-containers   --set image.tag=0.1.0   --set 'imagePullSecrets[0].name=dockerhub'
```
...
podman manifest push --all cac-multi   docker.io/YOURUSER/cards-against-containers:0.1.0
```
